...
- **`~/.ssh/config`, only when you ask.** `sshelf import` (or `Ctrl-o`) parses it and copies
...
Never in `hosts.toml` — that file is safe to commit and share. Never on a command line, so
