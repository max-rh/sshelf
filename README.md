...
config-free flags — and when the session ends you're back at your shell, not in a menu. The
...
doesn't exist, a missing agent socket, and an export that has drifted — and names the fix for
...
Compiled in September 2026 from each project's own README (and, for Termius, its site). A dash
means the project doesn't say.
...
| Holds tokens for other services | no | yes, one per cloud provider | no | yes, for sync | — |
| Network calls of its own | none | cloud provider sync | host metrics polling | sync, updates, host metrics | sync |
| Secrets custody | your OS keyring / `age` vault | keyring + password managers | — | its E2EE vault | its encrypted vault |
| Connect | `exec()` into your `ssh` | runs your `ssh` command | built-in terminal | built-in terminal | built-in terminal |
| Port forwards | yes, survive quitting | yes, live monitoring | — | yes | yes |
| File transfer | dual-pane SFTP | split-pane | two-panel SFTP | dual-pane, drag & drop | SFTP |
| Containers / metrics dashboards | no | yes | yes | yes | — |
