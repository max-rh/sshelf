...
with the same rules and the [same read-only, add-only promise](import.md#from-tailscale).
...
- [Exporting to SSH config](export.md): let plain `ssh`/`scp` and VS Code Remote use your
  hosts by name.
- [CLI reference](cli.md): scripting with `add`, `list --json`, `print-command`, completions.
