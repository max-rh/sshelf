...
- `tag:NAME` keeps only the hosts with that tag. Repeat it and combine it with text
  (`tag:prod tag:db` is an AND).
- `site:NAME` keeps only the hosts in that [site](sites-tags.md).
...
- Stored-password hosts in [vault mode](passwords-2fa.md#where-secrets-live) (with
  `$SSHELF_VAULT_PASSPHRASE` set), for the same reason: the master passphrase would have to
