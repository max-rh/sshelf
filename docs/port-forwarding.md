...
- Remote (`-R`) is a port *on the server* that tunnels back to something reachable from
  your machine. E.g. let someone on the server's network reach a dev server on your laptop.
- Dynamic (`-D`) is a local SOCKS proxy that routes traffic through the server.
...
connect does (keys/agent/ProxyJump, stored password, [site defaults](sites-tags.md)), then waits
