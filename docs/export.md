...
- Password-auth hosts export fine, but plain `ssh` can't read sshelf's keyring, so it
...
- Names that can't be Host patterns (containing `*`, `?`, `!`, `,`, `#`, or quotes) are
  skipped with a comment, since they'd otherwise match *other* hostnames. Names with spaces are
  quoted and work.
